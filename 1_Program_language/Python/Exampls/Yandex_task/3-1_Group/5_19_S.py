...
position = 0
while fuzze:
    if (listt[position].isalnum()) or (listt[position].startswith("-") and listt[position][1:].isdigit()):
        # Лучше не так, из за отрицательного числа(что не признаётся как число), лучше сделать список операций (и проверять по вхождению в этот список).
        position += 1
        continue
    else:
        result = None
        ...
        if len(listt) == 1: #Эта часть не нравится, здесь нужно сделать < 3 иначе будет бесконечный цикл.
            fuzze = False
        position = 0
