...
class meta(type):
    ...
    def __init__(target_Class, name_targetClass, parentClassList, dictOfAttr):
        ...
        print(super().__name__) # <<<<< Почему вывод ::: А ??? Как может быть у meta супертип это А ???
        print(super().__bases__)
        print(target_Class)
        print("It is __init__ of class meta END")
        #object.__init__(name_targetClass, parentClassList, dictOfAttr) # <<<<< ???
        type.__init__(target_Class, name_targetClass, parentClassList, dictOfAttr) # <<<<< ???
...
class A(metaclass=meta):
    ...
    def __call__(self, a, b):
        print("It is __call__ in A class a = %s , b = %s, param = %s " % (a, b, self.param))
