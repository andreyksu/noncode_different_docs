...
def values(a_func, a_start, a_end, a_step):
    x_range = np.arange(a_start, a_end + a_step, a_step)
    y_range = list(map(a_func, x_range))
    series = pd.Series(y_range, index=x_range)
    return series


def min_extremum(a_data):
    return a_data.idxmin()


def max_extremum(a_data):
    return a_data.idxmax()
