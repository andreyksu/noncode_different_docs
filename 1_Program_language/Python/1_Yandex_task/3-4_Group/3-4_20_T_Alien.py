# Это решение из интернета. Нашёл уже после того как сам решил.
# От сюда взял только идею "^" это "!="; "->" это "<=" ; "~" это "==""
...
def parse_expression(expression, variables):
    ...
    expression = expression.replace("(", "( ").replace(")", " )") # Что не нравится, то что фменяется только для скобок. А если перменная прижата к знаку операции?
    ...
    for item in expression.split():
        if item in variables:
            ...
            while stack[-1] != "(": #Вот это мне больше нравится чем моё.
