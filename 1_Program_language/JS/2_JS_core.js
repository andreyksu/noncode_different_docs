...
Как получить отицательное число через побитову операцию?
int y = ~x + 1
y == -x
*/
//=============================================================================
...
Область видимости:
	...
	В JS есть понятие видимость в функции, все переменные, объявленны внутри функции, видимы везде в теле функции и во вложенных функциях.
	
	var
		Переменная объявленная как var в for - так же видна во всей функции т.е. за блоком for (что отличается от C подобных языков).
*/
		function a(){
			...
			for (var k=0; k< 100; k++){
	...
	var
		/*
			- ограничена областью видимости функции.
				-- или круглой скобкой в которую может быть помещена функция.
			- её значение будет undefined если вы попытаетесь обратиться к ней до её объявления.
		*/
				var customer = "Joe";
				(function () { // customer поднимится до этого места, но еще значение не будет присвоено на этом этапе и соответственно следующая строчка выведет undefinde
					console.log("The name of the customer inside the function is " + customer); // Да эта переменная является локальной.
					if (true) {
						var customer = "Mary";
					}
				})();
		...
		/*
			- обрабатывается в самом начале работы функции или скрипта, если переменная является глобоальной. И не важно где var переменная объявляется.
				Тут говорят о "всплытии/hoisted" var всегда поднимается наверх. 
					Однако нужно разделять, объявление переменной и присваивании значения переменной. Объявление выполняется в начале работы функции, а присвоение при доходе до строчки.
		*/
...
//=============================================================================
// Так для JS можно. Все присоединения присоедениня валидны. А вот в TS нет.

		class MyStringArray {
		    // someStr = ''; // В js назначенное значение в конструторе this.someStr = someStr без объявления поля класса, поле становится доступно всем методам доступно всем методам.
		    // А вот в TS нужно объявить в классе, как в Java.

		    constructor(someStr) {
		        this.someStr = someStr; // Видимо в JS как в python а в TS как в java - нужно объявить поле класса.
		    }

		    innerAttahced(a) {
		        return this.someStr + "___innerAttahced___" + a;
		    }
		}

		MyStringArray.prototype['toPrototypeAttached'] = function (a) { // В TS так нельзя. Там все по феншую. Там пинтерфейсы.
		    return this.someStr + "___toPrototypeAttached___" + a;
		}


		let iii = new MyStringArray("New Instance");

		iii["a"] = "aa"; // В TS так тоже нельзя

		iii['toInstanceAttached'] = function (a) { // В TS так тоже нельзя.
		    return this.someStr + '___toInstanceAttached___' + a;
		}

		console.log(iii.a)
		console.log(iii.innerAttahced("VAR_A"))
		console.log(iii.toPrototypeAttached("VAR_A"))
		console.log(iii.toInstanceAttached("VAR_A"))
...
Объект:
	...
	Каждый объект имеет второй ОБЪЕКТ, ассоциированный с ним - это и есть ПРОТОТИП. И первый объект наследует свойства от этого прототипа (от этого объекта).
		Т.е. ПРОТОТИП это просто ОБЪЕКТ который предоставляет свойства и функции, для первого обоъекта. При этом любой объект, а не какой-то особенный.
		...
		Ссылка на прототип хранится в __proto__ - это сеттер и гетер для [[Prototype]]. Т.о. доступ/связь с прототипом осуществляется через эту ссылку.
			В др. статьях __proto__ - называют псевдонимом внутреннего свойства [[Prototype]]. Получить прототип объекта можно с помощью метода getPrototypeOf()

			Только в функции-конструктор есть поле prototype, который содержит поле __proto__, который является псевдонимом для [[Prototype]].
				У стрелочных функций нет prototype.

			Важно отличать конструктор и prototype (речь идёт в контексте функции-конструктора).
				prototype - это план объекта, который может состоять из различных частей - методов и переменных, а конструтор - только часть прототипа.
*/
					function Person(name, age) {
					    this.name = name;
					    this.age = age;
					    this.print = function(){ // Так лучше не делать. Лучше долавлять в prototype
					...
					Даст (должен дать):
						{constructor: ƒ}
							...
							[[Prototype]]: Object
							// Т.е. внутри prototype функции-конструктора лежит ОБЪЕКТ с двумя полями: с полем constructor и ссылка-прототип на Object. 
					*/
								
						// Т.к. поле constructor это часть прототипа. Соответственно, можно получить поле constructor через объект prototype.
						console.log(Person.prototype.constructor);
							const tom = new Person("Tom", 39);
							console.log(tom.constructor);

					// Изменим первую версию Person. Перенесём функцию в прототип (т.е. сделаем правильно). 
					// Так правильнее. Так как иначе при каждом создании объекта будет выполняться тело функции Person и будет выполняться эта функция. Перенеся в prototype - мы избегаем этого.

					function Person (name, age) {
						    this.name = name;
						    this.age = age;
						}
						// функция print определена как часть прототипа. Т.е. убрали из функции-конструктора и воткнули в ОБЪЕКТ прототип. ЗЫ. так и нужно - функции только в прототипе.
						Person.prototype.print = function(){
						    console.log(`Name: ${this.name}  Age: ${this.age}`);
						};

						p = new Person(...);
						p.__proto__ === Person.prototype // Даст true. Т.е. у instance есть поле __proto__ который ссылается на ОБЪЕКТ-prototype функции-конструктора а этот ОБЪЕКТ-prototype тоже содержит  __proto__ но т.к. мы не меняли его он указывает на Object

						console.log(Person.prototype); // Выведет объект что лежит в prototype (вернее на который ссылается prototype)
						Даст (должен дать):
							{print: ƒ, constructor: ƒ}
								print: ƒ ()
								constructor: ƒ Person(name, age) //Т.е. в прототипе теперь два поля. Конструктор и метод print
								[[Prototype]]: Object

					/*
					- При этом не важно где объявлен метод внутри конструктора или внутри прототипа. Будет/должно работать однинаково.
						-- При этом если один и тот же метод объявить и в конструкторе и в прототипе, то метод конструктора будет перекрывать метод прототипа.
					- Но вот если свойство будет помещено в прототип, то но будет видно всем объектам в одном состоянии (как static в Java).
						- Все логично, ведь объект прототипа один на все инстансы.
							Person.prototype.company = "SuperCorp";

					Т.е. при создании объекта-инстенса он должен иметь ссылку на некий другой объект от куда будет брать методы - это и есть прототип. 
						Свойства он должен брать в функции-конструкторе.
				...
				// А можно даже так. В prototype засунули объект и создали instance 'rabbit'. Соответственно этот объект перейдёт инстансу. И всё, после этого prototype уже не влияет на созданный instance 'rabbit'
					let animal = {
						eats: true
					};
					...
					function Rabbit(name) { // прототип по умолчанию Rabbit.prototype = { constructor: Rabbit };
						this.name = name;
					}

					Rabbit.prototype = animal; // Но похоже потеряли constructor???
					let rabbit = new Rabbit("White Rabbit"); //  rabbit.__proto__ == animal
					Rabbit.prototype = animall; // Это не влияет уже на созданный объект, а будет влиять на новые создаваемые объекты.
					console.log( rabbit.eats ); // true - т.е. не повлияла смена protutype
			...
			// Наследование для функций конструкторов:
				...
				Animal.prototype.speak = function() { // ОБЪЕКТ-prototype положили поле speak которой присвоили функцию
				    console.log(`${this.name} makes a noise.`);
				};

				function Dog(name) { // Это аналог вызова родительского конструтора. Мы передаём this Dog в Animal - чтоб тот тоже мог проинициализировать свои переменные и присвоить их this из Dog
				    Animal.call(this, name); // Call the parent constructor. Здесь мы this у Animal привязали к текущему объекту и выполнили тело функции.
				    .... // Other statements and fields
				}


				Dog.prototype = Object.create(Animal.prototype); // Т.е. здесь создаём пустой объект и устанавливаем этому пустому объекту прототип __proto__ равным Animal.prototype и закидываем этот новый созданный объект в свойство Dog.prototype - т.е. подменяем/перетераем старый ОБЪЕКТ-prototype новым объектом.
				Dog.prototype.constructor = Dog; // Set the constructor property. Т.е. мы в ОБЪЕКТ-prototype записали новый объект и т.о. потеряли/перетерли constructor. Здесь мы его возвращаем обратно.

				// Т.е. теперь в Dog.prototype лежит прототип указывающий на Animal.prototype и Dog.prototype.constructor указывающий на Dog.
!!!				// Dog.prototype играет роль только при создании объекта, дальше не влияет.
				d1 = new Dog(...)
				Dog.prototype = <newOtherObject>
				d2 = new Dog(...)
				// Такой ход не повлияет на d1  него по прежнему будет старый прототип. А вот у d2 уже новый.
...
!!!!		- Нужно обратить внимание, что prototype присутствует только у функций-конструкторов (Array - функция конструктор объектов). 
				-- По умолчанию "prototype" – объект с единственным свойством constructor, которое ссылается на функцию-конструктор.
!!!!				Rabbit.prototype == { constructor: Rabbit, __proto__ : Object.prototype }; 
					// Вот оно цепочка неследования. __proto__ указывает на объект, а у объекта может быть совй прототип и так вверх по цепочке.
			...
			Можно доработать встроенные прототипы добавив новые методы
				...
				// Значение prototype используется только при создании нового объекта (т.е. копируются из него поля). И дальшейшая работа должна идти через __proto__
					alert(this);
				};
				"BOOM!".show(); // BOOM!
...
!!!		Данные и методы хранятся только в объектах-инстенсах, а конструкторы лишь позвляют конфигурировать будущий объект.
...
!!!!!		При этом функция-конструктор должена содержать ислючительно данные, а все методы должны быть в prototype. 
				Обусловлено это тем, что при каждом вызове черзе new - выполняется тело функции, и соответственно для каждого вызова, будут создаваться ананимные функции, что объявлены в конструкторе (что даст падение по производительности и увеличение потребления памяти).
	...
	Для объектов:
		...
		rabbit.__proto__ = animal; // и снова __proto__ это лишь геттер и сеттер. Теперь rabbit имеет в качестве прототипа объект animal.
	...
	Варианты создания объектов:
		...
		3. var obj = Object.create(prot) - создает новый объект, с yказанием прототипа - по сути это есть ссылка на объект. Где create статическая функция.
			var o3 = Object.create(Object.prototype); - по сути полный аналог var o3 = {} или o3 = new Object()
				Object.create(...) первый аргумент указывает на прототип, что будет использоваться в будущем объекте. Второй аргумент - это объект.
    ...
    В JS - функция является объектом Function
        ...
        ~    Среди свойств объекта Function можно выделить следующие:
            ...
            Среди методов надо отметить методы call() и apply().
            	methodName.call(this, argsForFunction) а apply тоже с самое, только аргументы передаются в виде массива а не перечислением аргументов через запятую, как в call.
...
+++++
	...
	Функция - это тоже объект и ей можно оперировать так же как объектом (и отличие от обычной функции появляется только когда используется this).
		...
		Dog.prototype = Object.create(Animal.prototype); //Создаёт новый объект и присваивает его Dog.prototype. В этом новом объекте есть ссылка на прототип Animal.prototype.
		// Вот здесь важно!!! Так делать не нужно Dog.prototype = Object.create(Animal); т.е. нужно именно Animal.prototype.
		// А можно так!!! Dog.prototype = new Animal(); Но так лучше тоже не делать!!!!!
			// Т.к. свойства установленные в объекте прототипе видны во всех инстенсах объекта Dog. Т.е. поля прототипа для инстенса объекта являются аналогом static в Java.
			// If the Animal constructor initializes properties (e.g., this.name), those properties will be shared across all instances of Dog because they are part of the Dog.prototype object. This is usually not the desired behavior.		

		Dog.prototype.constructor = Dog; // Set the constructor property
		// Кажется, что в этом месте мы меняем и у Dog значение для поля constructor. Но на самом деле это не так. У нас создан новый объект и мы туда добавляем конструктор.
		// If you define a property on an Employee instance that has the same name as a property on Person.prototype, you are not modifying Person.prototype; instead, you are creating a new property on the instance itself. This is known as property shadowing.

		const dog = new Dog('Rex');
		dog.speak(); // Output: Rex makes a noise.
		/*
			Summary
				- Correct Usage: Use Object.create(Animal.prototype) to set up inheritance correctly, allowing instances of the child constructor to access methods defined on the parent's prototype.
				- Incorrect Usage: Avoid using Object.create(Animal) when setting up inheritance, as this will not establish the correct prototype chain and will lead to errors when trying to access inherited methods.
				- In summary, always use Object.create(Animal.prototype) when you want to inherit from a constructor's prototype in JavaScript.
		*/


//=============================================================================
//Хорошая статья - с разборами примеров
...
Promise
	...
	// Помните о том, что интерпретатор JavaScript всегда сначала выполняет синхронный код, а затем асинхронный. Асинхронный это тот который в then.
	// Помните, что метод resolve не прерывает выполнение функции. Код, стоящий за ним, по-прежнему будет выполняться (не дожидаясь выполнения того, что внутри then)
		var promise = new Promise((resolve, reject) => {
			console.log(" START promise body ");
		    setTimeout(() => {
		        resolve("result from Resolve"); // Вот она асинхронная часть. Остальное всё синхронное.
		    }, 3000);
		    console.log(" END promise body ");
		});
		...
		// end promise body
		// result from Resolve
		fetch(`https://swapi.dev/api/films/${id}/`).then(function (movies) { // в movies ответ сервера после выполнения.
			renderList(movies)
		})
	...
	*/
		...
		let prom = new Promise(function (res, rej){ //эта функция называется исполнитель, как только создается Promis эта функция сразу же начинает исполняться. При этом res и rej нужно вызывать в коде самостоятельно.
			...
			if (a=='a'){
				res("Успех"); // Вот асинхронное место.
			}else{
				rej(new Error()) // Вот асинхронное место.
			}
			consloe.log("endd"); // Вызов resolve не приостанавливает выполнение. И соответственно выполнится эта строка.
		})
		
		// Вот асинхронное место.
		// Если промис еще не закончил исполнение, then будет ждать окончания выполнения.	
		prom.then(funcForRes(...){...}, funcForRej(...){...})

		//.catch(f) это тоже самое, что prom.then(null, f)
		//.finally(f) похож на .then(f, f) // но обработчик вызываемый из  finally не имеет аргументов.
		
!!!!	// then - возвращает promise. Т.е. все что возвращается из then оборачивается в Promise и по этому можно делать цепочку then
			// Пример:
			new Promise(...).then(function(res){...; return someRes_1}).then(function(res){...; return someRes_2}) // второй(в этом случае последний) then будут выполняться для Promise результатом someRes_1;
		...
		// Не путать с prom1.then(..); prom1.then(...) в этом случае все относится к одному prom1.
	...
	Как правило в rej передается ошибка/исключение - рекомендуется. Да можно передать все что угодно как и в res - но рекамендуется объект Error.
...
// Пример использованрия и применения всё вместе. См. 4_Promise 1_...js
==================
// Пример НЕправильной (из за места добавления функции) но возможной работы с функциями как конструкторами.
	function A(){
        this.a = "aaa";
...
=======
    ...
    // Как говорят ребятки, в конструктор не нужно добавлять др. методы, в конструкторе должны содержаться только поля. 
    // Обусловлено это тем, что при каждой инициализации объекта через new будет выполняться тело фонуции-конструктора и будут создаваться каждый раз аноноимные функции в таком же количестве сколько их объявлено в конструкторе.
    "use strict";
    ...
    //A.prototype = someObj // А можно и так. Вот пример, когда в прототип устанавливается объекта.
    
    A.prototype = Object.create(Object.prototype, {firstProp: {value: "first"}, secondProp: {value: "second"}}); //это можно и не делать, добавлено просто для примера.
    //Object.create(proto, [descriptors]) – создаёт пустой объект со свойством [[Prototype]], указанным в аргументе - proto, и необязательными дескрипторами свойств descriptors. 
    ...
    B.prototype = Object.create(A.prototype); //Здесь по сути создали объект из "A.prototype"  и установили в качестве прототипа для B 
    //prototype заданный для конструктора используется только один раз, когда выполяется new. Дальше работа идет с __proto__, и prototype никак не влияет на созданный объект.
    // Т.е. если B.prototype = <otherObject> то объекты созданные до такого привоения через new B(...) будут иметь старый прототип, а новые инстансы уже новый прототип.
    B.prototype.constructor = B; //после изменения prototype нужно обновить и конструктор, иначе конструктор у B будет А.

    // ??? А почему не сделал для А аналогично constructor
...
import name, {name1} from './module.js'; // В node можно просто писать без пути.

Без скобок указывается, если в импортированном модуле сделано через default:
	export default name;
		При этом default может быть только один.

Если же name1 не экспортируется через default то нужно заключить в фигурные скобки.
	А можно еще так:
		export {sayHi, sayBye}; // Т.е. за раз экспортируем целевые поля/имена. Вместо каждого отдельного экспорта.
		export {sayHi as sH, sayBye as sB};
		//-----------------
...
// call и this для функций
	...
	function new_w(x){
	  console.log(this.someMethod() + " outer"); // Эта функция получает this при присваивании.
	  function new_ww(){
	    console.log(this.someMethod() + " inner"); // А Эта функция НЕ получает this из внешний функции. В это случае this == undefined. По этому нужно ипользовать call
	  }
	  new_ww(); // Так не работает.
	  new_ww.call(this); // Так работает.	  
	}
	...
	function new_w(x){
	  refff = this
	  console.log(this.someMethod() + " outer"); // Эта функция получает this при присваивании worker.mm = new_w;
	  function new_ww(){
	    console.log(refff.someMethod() + " inner"); // А эта фунцкия получает ссылку через refff
	  }
	  new_ww(); // Так и здесь работает.
	...
	let worker = {
	    ...
	    slow(x) {
	        console.log("Called with " + x + this.field);
	...
	func(' With_out_call '); // А тут ошибка; Для поля undefinde, а на методе падаем.

====================================================================================================================================================================================
Варианты чтения файла или json

import jsonData from './infoo_1.json' with {type: 'json'}
//import africaJson from './africa.json' assert { type: 'json' };

//-------------------------------------------

var jsonData = require('./infoo_1.json');

//-------------------------------------------

//synchronously
const fs = require('fs')
let jsonData = JSON.parse(fs.readFileSync('./infoo_1.json', 'utf-8'))
...
fs.readFile('infoo_1.json', 'utf-8', (err, data) => {
  if (err) throw err
...
//-------------------------------------------
 ...
 const jsonData = fs.readFileSync('/Users/joe/test.txt', 'utf8');

//-------------------------------------------

const fs = require('node:fs/promises');
fs.readFile('./infoo_1.json', { encoding: 'utf8' }).then((resss)=> console.log(resss))
