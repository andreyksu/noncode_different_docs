...
let obj = {
  toString(){ // Но для доступа к имени  нужно явно реализовывать этот метод. Допустим для let obj = {message:'Hello'}  toString() даст [object Object]
...
// Проверка на null : x!.toStirng()
	...
	function liveDangerously(x?: number | null) {
	  // No error
	  console.log(x!.toFixed()); // Зачем это нужно? После компиляции это пропадает и всё равно будет ошибка. Видимо только для компиляции. Но здесь тогда должно быть условие if(x) итд.
	}
...
// Проверка типа:
	...
	// Массив можно обяъявить и так Array<number> или number[] а проверить массив ли это можно так Array.isArray(x)
