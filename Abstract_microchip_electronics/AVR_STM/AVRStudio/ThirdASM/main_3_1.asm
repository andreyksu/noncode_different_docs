...
;#	1МГц/256=3906; 3906/255=15.31;  15.31*60=918 - Пределитель в 256 - соответствует 918 оборотам двигателя.	1с/122,55=62,32
;#	1МГц/128=7812; 7812/255=30.64;  30.64*60=1838 - Пределитель в 128 - соответствует 1838 оборотам двигателя.	1с/122,55=32,6,16
;#	1МГц/64=15625; 15628/255=61.27;  61.27*60=3676 - Пределитель в 64 - соответствует 3676 оборотам двигателя.	1с/122,55=16,32
...
;-------------------------------Устанавливаем регистры сравнения OCR1A(OCR1AH и OCR1AL). На самом деле их два таких регистра. Есть еще OCR1B. И можно их обоих задать. И прерывание будет идти от них двоих. Может быть удобно.
	...
	out		OCR1AL, temp
;-------------------------------
	ldi		temp, 0<<FOC2 | 1<<WGM20 | 1<<COM21 | 0<<COM20 | 1<<WGM21 | 0<<CS22 | 1<<CS21 | 1<<CS20
	out		TCCR2, temp

	ldi		temp, 0x3E		;2ms для 1MHz
	out		OCR2, temp
